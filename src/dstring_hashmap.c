...
{
	return find_equals_in_dmap(dmapp, ((key) ? (&get_dstring_pointing_to_cstring(key)) : NULL));
}
...
{
	insert_unique_in_dmap(dmapp, ((key) ? (&get_dstring_pointing_to_cstring(key)) : NULL), ((value) ? (&get_dstring_pointing_to_cstring(value)) : NULL));
}
...
{
	insert_duplicate_in_dmap(dmapp, ((key) ? (&get_dstring_pointing_to_cstring(key)) : NULL), ((value) ? (&get_dstring_pointing_to_cstring(value)) : NULL));
}
...
{
	return remove_from_dmap(dmapp, ((key) ? (&get_dstring_pointing_to_cstring(key)) : NULL));
}
