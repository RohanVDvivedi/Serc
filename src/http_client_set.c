...
client_set* new_http_s_client_set(const dstring* uri, SSL_CTX* ssl_ctx, unsigned int max_clients)
{
	uri uriv;
	init_uri(&uriv);

	if(-1 == parse_uri(&uriv, uri))
		goto ERROR;
	...
	char* hostname = get_byte_array_dstring(&(uriv.host));
	char* port = get_byte_array_dstring(&(uriv.port));
